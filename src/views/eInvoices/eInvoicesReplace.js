...
const EInvoicesReplace = () => {
  ...
  const saveChangeEInvoices = () => {
    ...
    axios
      .post("http://127.0.0.1:8000/api/v1/eInvoices/replaceEInvoices", data)
      .then((response) => {
        toast.success(response.data.message);
        document.getElementById("loading-container").style.display = "none";
      })
      .catch((error) => {
        toast.warn("Hóa đơn chỉ được đều chỉnh");
        console.log(error);
        document.getElementById("loading-container").style.display = "none";
      });
  };
    ...
    <>
      <div className="toolbar row justify-content-between">
        <p className="col-2 p-0 m-0 toolbar-title">
          <Link to={"http://localhost:3000/eInvoices/"}>Hóa đơn</Link> / Thay
          thế hóa đơn
        </p>
...
export default EInvoicesReplace;
